...
## Задание 4. Повышение оперативности и стабильности работы CRM
  ...
  2. Настроен Debezium на отправку данных в топик Kafka. Сделано в [init-crm-connect.sql](sql/init-crm-connect.sql).

  3. Настроен приём данных из топика Kafka в OLAP БД Clickhouse с помощью механизма KafkaEngine.
    Сделано в [init-crm-connect.sql](sql/init-crm-connect.sql).
...
Этот проект представляет собой комплексную систему для управления безопасной аутентификацией, 
генерацией аналитических отчётов и эффективной обработкой данных в распределённой архитектуре. 
Проект реализует современные подходы к безопасности, микросервисной/межсервисной архитектуре.
...
- Поддерживается **MFA (OTP)** — после ввода логина/пароля пользователь должен ввести одноразовый пароль из Google Authenticator. (было реализовано в одном 
из заданий, но далее не поддерживается)
- Реализован **PKCE** (Proof Key for Code Exchange) для защиты от перехвата кодов авторизации, также показано в одном из заданий.
...
Самая сложная задача было потрачено пару дней в поисках грааля с импортом и синхронизацией realm roles/ldap users
...
- **Фронтенд** — только UI.
- **bionicpro-auth** — управление сессиями и токенами, бизнес-логика отчётов и проверка прав доступа. Не стал создавать еще один сервис,
т к это все же песончница, а не выкатка на прод. Ну и сам сервис тогда можно было иначе назвать. 
...
- `bionic_etl_daily` — генерирует готовые отчёты по пользователям и сохраняет в **MinIO** и **ClickHouse**.
...
- При запросе:
    - Сервис `bionicpro-auth` проверяет наличие отчёта в MinIO.
    - Если нет — запускает генерацию (через Airflow). Важный момент, для проверки требуется выполнить задание на стороне airflow в ручную.
...
- **C# .net10** — основной язык для бэкенд-сервисов (`bionicpro-auth`).
...
- **Автоматизация**: Airflow запускает DAG'и по расписанию.
...
Проект BionicPRO демонстрирует зрелую архитектуру, сочетающую 
безопасность, производительность и современные практики DevOps и Data Engineering. Он решает реальные бизнес-задачи:
